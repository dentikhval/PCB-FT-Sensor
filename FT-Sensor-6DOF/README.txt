...
We'll employ the principle of assembling PCBs the same way as slot-together laser cutparts are assembled, just sliding the spokes into the slots in the base, and then inserting the center ring and rotating it in place. 
...
Once the structure assembly is done, the strain gauges can be glued in the areas marked on the silkscreen with a drop of superglue (preferably one that is intended for gluing flexible items), which will spread itself under the surface of the strain gauge. The thinner is the glue and the closer the entire strain gauge is to the board, the better the precision of the assembly will be. If the glue doesn't cover the entire area of the strain gauge, it will cause poor response from that sensor and distorted readings.
This is probably the most crucial part of the assembly that will define the precision and calibration of the resulting device. Remember: single strain gauges only measure elongation in one direction, so they must always be mounted along the path of the elongation / contraction, as marked on the board.
